...
- **Schedule and retention.** The Backup Schedule and Retention (days) settings are enforced by the 15-minute maintenance tick. A scheduled backup is taken when the newest `*.db` in `backup.dir` is older than the interval (a manual backup resets the clock). Retention deletes every `*.db` there older than the window by mtime (manual, scheduled and `pre_restore_*` alike), never the newest file.
