...
- **Go:** `gofmt` + `golangci-lint` (CI pins v2.11.3). Prefer the standard library, except for locks: declare `syncutil.Mutex`/`syncutil.RWMutex` (`Server/syncutil/`), never a raw `sync.Mutex`/`sync.RWMutex`. `syncutil` gains deadlock detection under `-tags deadlock`, and the `syncutil-locks` rule in `Server/invariants/` fails `go test` on a raw lock in `ws/` or `service/`.
