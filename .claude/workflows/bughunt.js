export const meta = {
  name: "bughunt",
  description:
    "Converging multi-round bug hunt: rotating lens families, single opus finder, opus refute-by-default verification, dry-threshold stop",
  whenToUse:
    "Hunting real bugs across the Go server, Tauri Rust backend, and TS client until consecutive rounds go dry. Not a security-only scan.",
  phases: [{ title: "Recon", detail: "haiku: churn + concurrency-surface inventory" }],
};
...
const remainingBudget = () =>
  budget.total
    ? budget.remaining()
    : BUDGET_TOTAL
      ? Math.max(0, BUDGET_TOTAL - budget.spent())
      : Infinity;
...
log(
  `config: maxRounds=${MAX_ROUNDS} dryThreshold=${DRY_THRESHOLD}${CUSTOM_LENSES ? ` lenses=custom(${CUSTOM_LENSES.length})` : ""} budget=${BUDGET_TOTAL ? Math.round(BUDGET_TOTAL / 1e6) + "M" : "NONE - cost ceiling disarmed"}`,
);
    ...
    verdicts: {
      ...
      items: {
        ...
        properties: {
          ...
          reason: {
            type: "string",
            description: "what refutes it, or what confirms it in the code",
          },
...
const GRAPH_ROWS = (Array.isArray(ARGS.graph) ? ARGS.graph : []).filter(
  (r) => r && typeof r.file === "string",
);
...
const covered = new Set(
  HAS_INVENTORY ? GRAPH_ROWS.filter((r) => r.examined).map((r) => r.file) : [],
);
...
const uncoveredCount = () =>
  HAS_INVENTORY ? INVENTORY.reduce((n, f) => n + (covered.has(f) ? 0 : 1), 0) : 0;
if (HAS_INVENTORY)
  log(
    `coverage: inventory=${INVENTORY.length} preCovered=${PRE_COVERED} risky=${RISKY_FILES.length}`,
  );
...
function drawExploreFiles() {
  ...
  const avail = pool.filter(
    (f) => !exploreConsumed.has(f) && !covered.has(f) && !seen.some((s) => s.file === f),
  );
  const files = [];
...
function exploreLens(i) {
  ...
  return {
    key: `explore-${i}`,
    prompt:
      `${src} Read each one IN FULL with fresh eyes. Hunt every class: concurrency and ` +
...
while ((uncoveredCount() > 0 || riskySweepPending() || dry < DRY_THRESHOLD) && round < MAX_ROUNDS) {
  if (BUDGET_TOTAL && remainingBudget() < ROUND_BUDGET_FLOOR) {
    ...
    log(
      `Budget floor reached (${Math.round(remainingBudget() / 1000)}k left) - stopping before round ${round + 1}`,
    );
    break;
  }
  ...
  const lenses = family.filter(
    (l) => (sweepingNow && /^explore-/.test(l.key)) || (cleanStreak[l.key] || 0) < 2,
  );
  if (!lenses.length) {
    dry++;
    roundStats.push({
      round,
      family: familyName(round),
      lenses: 0,
      candidates: 0,
      fresh: 0,
      confirmed: 0,
  ...
  const lensResults = await pipeline(
    lenses,
    (lens) =>
      agent(finderPrompt(lens, rnd), {
        label: `r${rnd}:hunt:${lens.key}:opus`,
        phase: `Round ${rnd}`,
        model: "opus",
        effort: "high",
        schema: FINDINGS,
    ...
    async (r) => {
      ...
      const absorb = (vs) => {
        for (const v of vs || []) {
          ...
          if (idx === -1) {
            ...
            log(
              `r${rnd} ${lens.key}: verifier verdict "${v.title}" (${v.file}:${v.line}) ${claimed ? "duplicates an already-claimed candidate" : "matched no candidate"} - dropped`,
            );
            continue;
          }
      ...
      const v1 = await agent(verifyPrompt(lens.key, fresh), {
        ...vopts,
        label: `r${rnd}:verify:${lens.key}`,
      });
      absorb(v1 && v1.verdicts);
      if (!v1) counts.verifierNull++;
      if (unmatched.length) {
        const v2 = await agent(verifyPrompt(lens.key, unmatched.slice()), {
          ...vopts,
          label: `r${rnd}:verify:${lens.key}:retry`,
        });
  ...
  for (const r of lensResults.filter(Boolean)) {
    ...
    for (const { v, cand } of r.matched) {
      ...
      const rec = {
        ...
        line: v.line,
        title: v.title,
        status: v.refuted ? "refuted" : "confirmed",
      };
      if (seen.some((p) => isDup(rec, p))) {
        counts.suppressedRun++;
        continue;
      } // cross-lens same-round duplicate
      seen.push(rec);
    ...
    if (!r.finderFailed && !r.unmatched.length)
      cleanStreak[r.lens.key] = lensConfirmed > 0 ? 0 : (cleanStreak[r.lens.key] || 0) + 1;
    perLens[r.lens.key] = {
      candidates: r.unionCount,
      fresh: r.fresh.length,
      confirmed: lensConfirmed,
      refuted: lensRefuted,
      unverified: r.unmatched.length,
    };
  ...
  roundStats.push({
    ...
    refuted: newRefuted,
    dryEligible: eligible,
    dryAfter: dry,
    severity: sevMix,
    perLens,
    filesTouched: filesTouched.size,
    filesNew: filesNew.size,
    ...counts,
    spentBefore,
    spentAfter: budget.spent(),
  });
  log(
    `Round ${round} (${familyName(round)}): ${newConfirmed} confirmed, ${newRefuted} refuted, dry=${dry}${eligible ? "" : " (ineligible)"}`,
  );
  ...
  if (HAS_INVENTORY && familyName() === "adaptive" && uncAfter > 0 && newConfirmed === 0) {
    ...
    if (coverageStall >= 2) {
      ...
      log(
        `Coverage stalled: uncovered=${uncAfter} did not shrink for 2 adaptive rounds - stopping`,
      );
      break;
    }
...
const runStats = {
  config: {
    ...
    customLenses: !!CUSTOM_LENSES,
    knownCount: (ARGS.known || []).length,
    graphRows: GRAPH_ROWS.length,
    budgetTotal: BUDGET_TOTAL,
  },
  coverage: HAS_INVENTORY
    ? {
        inventory: INVENTORY.length,
        preCovered: PRE_COVERED,
        covered: INVENTORY.length - uncoveredCount(),
        uncoveredAtStop: uncoveredCount(),
      }
    : null,
...
function buildReport() {
  ...
  for (const f of confirmedSorted) {
    ...
    lines.push(
      `\`${f.file}:${f.line}\` - lens \`${f.lens}\`, round ${f.round}, confidence ${f.confidence}`,
      "",
    );
  ...
  if (unverifiedFinal.length) {
    ...
    for (const u of unverifiedFinal)
      lines.push(`- \`${u.file}:${u.line}\` ${u.title} (lens \`${u.lens}\`, round ${u.round})`);
    lines.push("");
  }
  lines.push(table);
  ...
  lines.push(
    "| round | spent | files (new) | suppressed ledger/run | finder null/empty | verifier null |",
  );
  lines.push("|---|---|---|---|---|---|");
  for (const s of roundStats)
    lines.push(
      `| ${s.round} | ${s.spentAfter - s.spentBefore} | ${s.filesTouched} (${s.filesNew}) | ${s.suppressedLedger}/${s.suppressedRun} | ${s.finderNull}/${s.finderEmpty} | ${s.verifierNull} |`,
    );
  return lines.join("\n");
}
