...
| 2   | build inputs pinned or auto-reviewed | **Partial** (question 2): five Dependabot roots plus `github-actions`, every remote `uses:` SHA-pinned, lockfiles authoritative — recorded, not rebuilt. The two floating toolchain channels (Go patch, Rust `stable`) are traceable through the SBOM, not pinned, until the owner answers | `.github/dependabot.yml`, the SBOM's `metadata.tools`/Go version field                |
...
| The compose file's LiveKit image is reviewed by that `docker` block               | **Corrected** | `Server/docker-compose.yml:66` `livekit/livekit-server:v1` is a floating major tag, and GitHub documents `docker-compose` as its **own** package ecosystem, separate from `docker`. The server's own download pins `DefaultLiveKitVersion = "1.13.5"` (`Server/ws/livekit_download.go:32`, "bump deliberately with releases"); compose disagrees with it                                                                                            |
...
| Build toolchains are pinned                                                       | **Corrected** | Go: `release.yml:261` `go-version: "1.26"` floats to the newest patch; `Server/go.mod:5` `toolchain go1.26.7` is a **floor**, not a pin. Rust: `dtolnay/rust-toolchain@… # stable` (`release.yml:97,164,355`) resolves whatever `stable` is that day; no `rust-toolchain.toml` exists. Neither is auto-reviewable; the SBOM records the Go version actually used, which is what "traceable" can honestly mean here                                  |
...
| Docs tell an owner how to verify a download                                       | **Refuted**   | `docs/deployment.md:780-782` describes what the **updater** verifies; `README.md:63` lists "checksums, signatures, and a full source snapshot"; no page shows `sha256sum -c`, `minisign -V` or any manual step. Task 5 adds one section                                                                                                                                                                                                             |
| A Dockerfile change is built on every PR                                          | **Corrected** | `ci.yml:656-657` `Server Docker Build (verify)` runs only when `github.ref_name == 'main' \|\| github.base_ref == 'main'`; on a dev PR it is skipped (`docs/contributing.md:210-212`). The digest pin is proven by `nightly-docker-smoke.yml:64-78` on `dev` or a local `docker build`, not by the PR                                                                                                                                               |
...
- **The digest pin is measured before it is claimed.** B6-2's "the layers are
  ...
  made equal.
- **Toolchain drift is recorded, not fought.** The Go patch and the Rust
  `stable` channel float; pinning them is an owner question (below) because
  CI would have to move with them. The SBOM names the Go version that built
  each asset, so the input is traceable even where it is not pinned.
...
| Pinned Go tool               | `.github/workflows/ci.yml:140`                | `go install <module>@vX.Y.Z` — exact version, sumdb-verified, no action                                                              |
| Digest-checked download      | `.github/workflows/ci.yml:426-439`            | version + sha256 in `env:`, `sha256sum -c` before use, comment saying why                                                            |
...
| Assets from disk, not a list | `.github/workflows/release.yml:676-691`       | "Built from the files on disk, not a hard-coded list" — the attestation subjects are globs over the staging dirs for the same reason |
| Operator recipe              | `docs/deployment.md:478-486`                  | numbered shell steps with a comment per line saying why, `--format '{{index .RepoDigests 0}}'` shape                                 |
...
| `.github/dependabot.yml`                                     | UPDATE | a `docker-compose` block on `/Server`, same shape as the `docker` block                                                                                                                                                                                                                 |
...
- **Action**: branch `feat/b6-12-signed-traceable-release` from `dev`. Flip
  the PRD row to `in-progress` with this plan linked. Confirm with
  `git diff --stat dev...feat/b6-10-operational-measurements` what that
  branch touches (`Server/api/metrics_handler.go` +test, `Server/api/router.go`,
  `Server/db/db.go`, `Server/scripts/k6/ws-load.js`, `docs/api.md`,
  `docs/deployment.md:738-756`, the PRD) — the overlap with this plan is
  `docs/deployment.md` and the PRD, rebased after B6-10 merges — and read
  B6-11's plan for `upgrade-rehearsal.yml` — this plan does not edit that
  file, `release.yml:497-502` only calls it.
...
- **Action**:
  1. `Server/Dockerfile:8` →
     ...
     Re-resolve both at implementation time with
     `docker buildx imagetools inspect <ref> --format '{{.Manifest.Digest}}'`
     (the digests above were resolved 2026-09-15 and are manifest-list
     digests, so `--platform` selection still works). One comment above each:
     the tag is for humans, the digest is what builds, Dependabot's `docker`
     block moves both together (`dependabot.yml:61-64` already says the
     builder tracks `go.mod` and `setup-go`).
  2. `Server/docker-compose.yml:66` → `image: livekit/livekit-server:v1.13.5`
...
- **Gotcha**: the PR's CI does not build the Dockerfile (`ci.yml:656-657`,
  main-only). Prove the pin with a local `docker build Server/` on both
  digests and, after merge to `dev`,
...
- **Validate**: `docker build --platform linux/amd64 Server/` succeeds;
  ...
  LiveKit tag; `npm run check:hygiene` (actionlint and Prettier over the
  YAML).
...
- **Action**:
  1. In `release-server` (`release.yml:231-322`), after the smoke step
     ...
     `v1.12.0` is the newest release on 2026-09-15 (re-check with
     `go list -m -versions github.com/CycloneDX/cyclonedx-gomod` before
     merging) and is written literally, exactly as `govulncheck@v1.1.4` is at
     `ci.yml:140`. `app`
     ...
     same artifact (`path:` takes a list), so it lands in `windows/` or
     `linux/` beside its asset at publish time (`:636-648`) and is picked up
     by the checksum step (`:668-669`), the attestation glob and
     `gh release create` (`:781`) with no further wiring. The manifest loop
     globs `chatserver*.exe` and `chatserver-linux-*.tar.gz` (`:688`) and is
     untouched.
...
- **Action**: `docs/deployment.md` gains "Verifying a download" after the
  Auto-Update section (`:772-815`), in the numbered-recipe shape of `:478-486`:
  ...
  `README.md:63` → "(binaries, checksums, signatures, SBOMs, a signed
  provenance attestation and a full source snapshot per release)".
  `CHANGELOG.md` Unreleased: one short block at the end per the style rule
  (`CHANGELOG.md:37-40`) — releases now carry attestations and SBOMs; the
  verify command.
...
| `cyclonedx-gomod app` fails on the Windows ARM64 runner                                                        | Low        | Medium | It is a pure-Go module; if a row fails, that row's SBOM is skipped with `::warning::` and the release proceeds — an SBOM is evidence, not a gate, in this milestone                                    |
...
- **Pinning the Go patch and Rust channel** — an owner question below; CI's
  `setup-go` and `rust-toolchain` lines would have to move in lockstep.
...
Server/go.mod` in `release.yml` makes the release build's Go version
   exact and Dependabot-reviewed (gomod block), at the cost of CI testing a
   possibly newer patch than the release builds. Rust: `dtolnay/rust-toolchain`
   at `release.yml:97,164,355` resolves `stable` on the day; a
   `rust-toolchain.toml` in `Client/src-tauri/` is one file, and the action
   honours it, so the pin is Dependabot-invisible but reviewable in git. This
   plan records the Go version in the SBOM and pins neither; until the owner
   answers, summary row 2 and its acceptance box are **partial**.
3. **Is the SBOM a gate or evidence?** This plan: evidence — a failed SBOM
   step warns and the release proceeds. Making it a gate is one `if:` change.
...
- [ ] Build inputs pinned or auto-reviewed — **partial** until question 2 is
      answered: actions, lockfiles and Dependabot roots yes; the Go patch and
      the Rust `stable` channel float, traceable only through the SBOM
