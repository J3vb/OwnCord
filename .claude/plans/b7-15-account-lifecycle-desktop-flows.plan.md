...
Each milestone of the split (15a server-info and sign-up, 15b recovery, 15c
deletion disclosure and support bundle) writes its own row when it lands.

| Milestone | Scope                                                                                                                                                                                              | Evidence                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| --------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| B7-15b    | Tasks 4, 5, 6 and the recovery e2e: recovery kit enrol/status/show-once, `/auth/recover` from the connect page (kit secret or owner credential), emergency codes at the 2FA box, code regeneration | `Client/tests/unit/login-form-recovery.test.ts` (2FA box widened: `maxlength` 11, `inputmode` text, both shapes accepted, a wrong code keeps the overlay, double Enter sends once; recover form validates, sends once, wipes on success/cancel); `Client/tests/unit/main.test.ts` (a recovered session is signed in through `completeLogin`, a wrong emergency code keeps the partial token); `Client/tests/unit/totp-settings.test.ts` (kit status badges, enrol shows the secret once, regenerate replaces the set, Done / tab switch / closing the overlay wipe the DOM — observed red with the `hide()` abort removed); `Client/tests/unit/recovery-secrets.test.ts` (planted kit secret, owner credential, codes and new password reach no log entry, console call, Web Storage, IndexedDB or Cache API with the real logger at debug; a negative control proves the detector fires, and it was observed red with the request body added to `api.ts`'s debug log); `Client/tests/unit/api.test.ts` (request shapes); `Client/tests/e2e/recovery-flow.spec.ts` (recover → signed in → new kit → regenerated codes gone after closing settings; an emergency code completes a 2FA sign-in) |
