...
- **Action**: write `docs/plans/hp-6-scorecard-<date>.md` with every section
  ...
  - the Success-Metrics table (`prd.md:66-75`) with "Actual" and "Evidence"
    columns, the TLS row pre-filled "unmeasured — decision 1";
...
- **Action**: with the record and every milestone's evidence:
  - Exit-gate map: each row's Evidence cell gets the PR, the run id, the test
    ...
    decision 1"; pending rows say "pending, not signed around" or "signed
    around — decision n" per the owner;
...
1. **Who is the unfamiliar owner?** A real person with sysadmin skill and no
   ...
   does not stall like a person and reads every page at once. This plan
   proposes: **a person for T1–T3, T6, T7 and T9** (the hands-on tasks), and
...
2. **The TLS Success-Metrics row: accepted limitation, or wait?** The PRD
   makes this the owner's call at the hold point (`prd.md:173-177`). This plan
   proposes **accepted limitation**, worded: "TLS mode matrix — unmeasured.
