...
- **Writes and conflicts** (`Queue.ts`). One write at a time, even across a
  ...
  leaves it or changes the filter. The report counts as the reader's own
  write from the moment the POST is sent, so a `mod_queue` re-read that lands
  before the server's 204 (the server broadcasts first) keeps it open too,
  but only until the answer: if that write fails, the quiet note is withdrawn
  and the report is treated like any other that left the list. A report
  that leaves the list for any other reason (another moderator's write seen
  in a background read, a lost 409 race, a filter change) still closes with
