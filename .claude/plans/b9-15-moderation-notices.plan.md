...
- **Handlers** (`features/safety/wsHandlers.ts`, registered in
  ...
  `mod_action` adds a warning or sets/clears the timeout and toasts once;
...
- **Timeout inline (Q4):** the composer shows "You can't send messages until
  ...
  `ChannelSidebar`), and a reaction click is refused with "You can't add
  reactions until …" before it is sent. The time is the server's `expires_at`.
...
- Outside the file table, each minimal: `ChannelController.ts` and
  `ChannelSidebar.ts` (Q4's inline timeout states), `main.ts` (sign-out
  ...
  Account).
- Reactions refuse with a toast rather than rendering every reaction chip
  disabled: the chips are shared message-list rendering, and the refusal
  names the same server expiry. The composer and voice join are inline.
...
| Unit            | `npx vitest run --maxWorkers=4`                                                                                                             | all pass, incl. `src/features/safety/safety.test.ts` (19) and the new cases in `channel-controller`/`channel-sidebar` |
...
| Budgets         | `npm run build:budget && npm run check:budgets`                                                                                             | MainPage 59,813 B / 60,000; startup 89,796 B / 91,000 (no budget raised)                                              |
