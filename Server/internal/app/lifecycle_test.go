...
func TestSeedHubReplayState_ForcesFullResyncForOfflineClient(t *testing.T) {
	...
	hub, hubErr := ws.NewHub(ws.HubOptions{DB: database, Limiter: limiter, Settings: service.NewSettingsService(database), Readers: ws.DBReaders(database), Voice: service.NewVoiceService(database)})
	if hubErr != nil {
...
func TestRunStartEventPersistence_DisabledMode_StaleLastSeqForcesFullResync(t *testing.T) {
	...
	hubOld, hubErr := ws.NewHub(ws.HubOptions{DB: database, Limiter: limiter, Settings: service.NewSettingsService(database), Readers: ws.DBReaders(database), Voice: service.NewVoiceService(database)})
	if hubErr != nil {
		t.Fatalf("ws.NewHub: %v", hubErr)
	}
	go hubOld.Run()
	...
	hubNew, hubErr := ws.NewHub(ws.HubOptions{DB: database, Limiter: limiter, Settings: service.NewSettingsService(database), Readers: ws.DBReaders(database), Voice: service.NewVoiceService(database)})
	if hubErr != nil {
		t.Fatalf("ws.NewHub: %v", hubErr)
	}
	go hubNew.Run()
