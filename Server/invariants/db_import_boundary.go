...
var DBImportAllow = map[string]DBImportEntry{
	...
	"api/dm_request_handler.go":       {"adapter", "", "message-request response types; the service owns the calls"},
	...
	"api/report_handler.go":           {"adapter", "", "db.User from the auth context only; ReportService owns every call"},
