-- Rollback of the B5-8 report-intake migration (048).
--
-- Cost: the queue and every outcome are gone, including the unlinkable
-- outcome rows decision 7 exists to keep. A report about an account that
-- has since erased carries the last durable trace that the report ever
-- happened -- dropping this table drops that trace along with everything
-- else, which is a real loss and not merely inconvenient.
--
-- Order: revoke the bit the migration granted BEFORE the tables drop (the
-- grant is a role edit, unconnected to the tables, but reversing the
-- migration in the opposite order it applied keeps this file readable
-- top-to-bottom against 048_reports.sql), then drop child-to-parent so the
-- foreign keys never point at a table that is already gone mid-rollback,
-- and delete the schema_versions row last so an aborted run cannot clear
-- the tracker before the schema change lands.
UPDATE roles SET permissions = permissions & ~4194304 WHERE id = 3 OR name = 'Moderator';

DROP TABLE IF EXISTS report_notes;
DROP TABLE IF EXISTS report_evidence;
DROP TABLE IF EXISTS reports;

DELETE FROM schema_versions WHERE version = '048_reports.sql';
