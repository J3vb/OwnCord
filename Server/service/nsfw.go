...
func (s *NSFWService) checkVisible(ctx context.Context, userID, channelID int64) (*db.Channel, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("%w: failed to resolve permissions", ErrInternal)
	}
...
func (s *NSFWService) Acknowledge(ctx context.Context, userID, channelID int64) error {
	ch, err := s.checkVisible(ctx, userID, channelID)
	if err != nil {
		return err
	}
	if !ch.NSFW {
	...
	if err := s.st.AcknowledgeNSFW(ctx, userID, channelID); err != nil {
...
func (s *NSFWService) Revoke(ctx context.Context, userID, channelID int64) error {
	if _, err := s.checkVisible(ctx, userID, channelID); err != nil {
		return err
	}
