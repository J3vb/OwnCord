...
// platform/desktop/settings.ts — is counted at 39 rather than 40.
...
// Both spellings count: 33 `#[tauri::command]` plus 12
// `#[tauri::command(async)]`. Matching the exact bracket form undercounts to 33.
...
describe("platform-contracts count table matches the tree", () => {
  it("counts what the table says it counts", () => {
    ...
    expect(invokeNames).toBe(39);
    expect(commandHandlers).toBe(45);
  });
