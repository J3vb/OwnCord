...
// platform/desktop/settings.ts — is counted at 47 rather than 48.
...
// Both spellings count: 41 `#[tauri::command]` plus 12
// `#[tauri::command(async)]`. Matching the exact bracket form undercounts to 41.
...
describe("platform-contracts count table matches the tree", () => {
  it("counts what the table says it counts", () => {
    ...
    expect(invokeNames).toBe(47);
    expect(commandHandlers).toBe(53);
  });
