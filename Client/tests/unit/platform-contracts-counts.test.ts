...
// platform/desktop/settings.ts — is counted at 37 rather than 38.
...
// Both spellings count: 31 `#[tauri::command]` plus 12
// `#[tauri::command(async)]`. Matching the exact bracket form undercounts to 31.
...
describe("platform-contracts count table matches the tree", () => {
  it("counts what the table says it counts", () => {
    ...
    expect(invokeNames).toBe(37);
    expect(commandHandlers).toBe(43);
  });
