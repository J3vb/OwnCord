...
// platform/desktop/settings.ts — is counted at 43 rather than 44.
...
// Both spellings count: 37 `#[tauri::command]` plus 12
// `#[tauri::command(async)]`. Matching the exact bracket form undercounts to 37.
...
describe("platform-contracts count table matches the tree", () => {
  it("counts what the table says it counts", () => {
    ...
    expect(invokeNames).toBe(43);
    expect(commandHandlers).toBe(49);
  });
