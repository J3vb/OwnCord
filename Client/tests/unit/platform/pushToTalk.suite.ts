...
export function describePushToTalkSuite(makeSubject: () => Promise<PushToTalkSubject>): void {
  describe("PushToTalk", () => {
    ...
    describe("captureKeyPress", () => {
      it("resolves the captured virtual key code", async () => {
        ctx.native.captureSucceedsWith(0x41);
        await expect(ctx.subject.captureKeyPress()).resolves.toBe(0x41);
      });

      it("rejects when the native capture command fails", async () => {
    ...
    describe("init", () => {
      it("resolves without starting native polling when no key is configured", async () => {
        await expect(ctx.subject.init()).resolves.toBeUndefined();
      });
    });

    describe("stop", () => {
      it("resolves safely when nothing is bound", async () => {
        await expect(ctx.subject.stop()).resolves.toBeUndefined();
      });
    });

    describe("updateKey", () => {
      it("resolves when disabling an unbound key", async () => {
