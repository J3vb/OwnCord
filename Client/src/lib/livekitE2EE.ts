...
export class E2EEManager {
  ...
  private async handleAnnounceInner(
    ...
    try {
      ...
      if (!(await this.verifyPeerAnnounce(userId, publicKeyBase64, signatureBase64, isCurrent))) {
      ...
      if (!isDuplicate) {
        if (retiredKey) this.retirePeerKey(userId, retiredKey);
