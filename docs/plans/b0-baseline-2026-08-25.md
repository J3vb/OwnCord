...
| Docker smoke | **unavailable** — no daemon | measured (ENV-02) |
| Server coverage | 74.6% aggregate | **carried**, not re-measured |
...
| G-04 | P1 | open | Superseded plans still claim "0 open findings"; needs the active-plan index. |
| ENV-02 | — | blocked | Docker smoke unavailable locally (no daemon). Needs Docker Desktop or CI evidence. |
...
- Step 5: Docker smoke (blocked on a daemon — ENV-02).
...
- Step 8: full reconciliation of the 38 open `OC-*` ledger records and the
  remaining register rows. The ledger count was verified as
  **306 fixed / 38 open / 3 declined / 1 duplicate = 348**, matching the
  register; adjudicating the 38 individually is outstanding.
- Step 9: a re-measured server coverage figure (74.6% is still carried).
