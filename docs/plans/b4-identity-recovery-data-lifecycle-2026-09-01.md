...
closed — the login lockout key folds like the account lookup); **HP-4 scorecard opened 2026-09-02** (branch `docs/hp-4-scorecard`; the five baseline drills green on snapshot copies, the schema drafts with rollbacks, the failure-model map and six recorded decisions; awaiting the owner's signature). Next B4-5 → B4-6, and after HP-4's signature B4-9 → B4-10 → B4-11. _Update this line — not only the step table — as steps
