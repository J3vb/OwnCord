...
was removed or loosened); that revision was re-run in CI at the PR head
`e485deea` on run
[36127046517](https://github.com/J3vb/OwnCord/actions/runs/36127046517), whose
`Client E2E (real server and media)` and `Client E2E (Windows native)` jobs
both passed. B9-26 merged as [#1797](https://github.com/J3vb/OwnCord/pull/1797).
...
| B   | Retention disclosed → reported + appealed subject self-erases → observer's content and membership gone, report outcome survives      | BPR-052/BPR-054 client half, BPR-070, BPR-090 | owner (alice), reporter (carol), subject (bob) | —                                                           | online throughout    | fullstack (real Go server) | Pass at `66da915a` (11.4 s); tightened revision passed at `e485deea` in run 36127046517                                                                                                                                                                                                                                                                                                                                   |
...
| F   | Transport cut mid-session → B9-25 notice says this server is unreachable + Retry → reconnect, state kept                             | BPR-090, BPR-092                              | member (bob)                                   | —                                                           | **cut and restored** | fullstack (real Go server) | Pass at `66da915a` (5.9 s); tightened revision passed at `e485deea` in run 36127046517                                                                                                                                                                                                                                                                                                                                    |
| G   | Member refused the queue, a queue action and another account's appeal, in UI and by the server                                       | BPR-071                                       | owner (alice)                                  | member (bob): queue, assign, close, list and decide all 403 | online throughout    | fullstack (real Go server) | Pass at `66da915a` (6.5 s); tightened revision passed at `e485deea` in run 36127046517                                                                                                                                                                                                                                                                                                                                    |
| H   | Labelled channel's evidence waits for the moderator's own acknowledgement; revoke takes it away                                      | BPR-063, BPR-071                              | owner (alice), reporter (carol)                | —                                                           | online throughout    | fullstack (real Go server) | Pass at `66da915a` (6.6 s); tightened revision passed at `e485deea` in run 36127046517                                                                                                                                                                                                                                                                                                                                    |
| I   | Integrated accessibility matrix: joined inbox + Moderation Center at 940×500 with 20 px text, contrast, keyboard, reduced motion     | BPR-091                                       | member (bob), owner (alice)                    | —                                                           | online throughout    | fullstack (real Go server) | Pass at `66da915a` (7.0 s); tightened revision passed at `e485deea` in run 36127046517                                                                                                                                                                                                                                                                                                                                    |
| J   | External-content consent gates the native broker; warm re-entry after consent does not refetch; destinations confined through logout | BPR-061, BPR-062, BPR-092 (desktop)           | member (alice)                                 | —                                                           | online throughout    | native (Windows WebView2)  | **Pass in the `client-native` job** (`Client E2E (Windows native)`, CI run [36127046517](https://github.com/J3vb/OwnCord/actions/runs/36127046517) on `fm/b9-26-impl`, head `e485deea`), which builds the real Windows app and runs `native-core` including `Client/tests/e2e/native/b9-journeys.spec.ts`. Not runnable on a headless Linux host, so the local run was not made; the CI job is the desktop qualification. |
...
on this headless Linux host, and the `client-native` job passed on CI run
36127046517 (head `e485deea`), so the row is desktop-qualified there.
...
- The owner's OS 200 % zoom check is **no longer an owner item**: it was replaced
  by the automated reflow evidence in `Client/tests/e2e/b9-zoom.spec.ts` (owner
  decision 2026-09-25, "automate the zoom"). Journey I covers the automatable
  940×500 / 20 px reflow and contrast; `b9-zoom.spec.ts` covers the 200 % zoom
  reflow per lane.
