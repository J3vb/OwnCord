...
| B7-17    | Desktop artifact matrix and smoke                      | An owner can install, boot, connect, update, roll back, use media, and recover an account on Windows x64/ARM64 and Linux x64/ARM64 builds, all exercised by CI, not just built; Windows ARM64 tries the native `windows-11-arm` runner first (decision 4)                                                                          | pending  | —                                                                                        | 12                                 |
