...
**Reconciled — first pass (2026-09-18).** HP-6 is not yet signed (no
...
against the first beta tag (D-02), so this pass records the rows the exit owes
rather than a signature that does not exist yet. The TLS block is the
**accepted limitation D-01** (B6-3 – B6-5 move to B11; a reverse proxy is the
recommended HTTPS path). B6-12 stays `in-progress` and its workstream-15
rehearsal row stays pending: the tag run that proves B6-1's three remaining
acceptance rows has not happened. The traceability landings for BPR-011/012/030
and the blocked sentences for BPR-014/015/016 are in the traceability document;
the roadmap's B6 workstreams and exit/evidence bullets carry the D-01 narrowing;
and the README status lines are out of this row's scope (the plan index owns
them). Nothing here claims a signed HP-6.

| Item                                                           | Verdict             | PR(s) / `dev` SHA                                             | Stale today? | Wording target                                                                 | Edited this pass?                   |
| -------------------------------------------------------------- | ------------------- | ------------------------------------------------------------- | ------------ | ------------------------------------------------------------------------------ | ----------------------------------- |
| Workstream 1 (release assets)                                  | shipped             | B6-1 #1580                                                    | no           | RC tag run id                                                                  | no — final pass                     |
| Workstream 2 (Docker images)                                   | shipped             | B6-2 #1583                                                    | no           | RC tag run id                                                                  | no — final pass                     |
| Workstream 3 (HTTPS/WSS modes)                                 | deferred            | B6-3/4 (open)                                                 | no           | dated deferred sentence, HP-6 decision 1                                       | **yes** (roadmap WS3)               |
| Workstream 4 (port-forward honesty)                            | shipped             | B6-6 #1588                                                    | no           | —                                                                              | no                                  |
| Workstream 5 (cert lifecycle)                                  | deferred            | B6-5 (open)                                                   | no           | dated deferred sentence, HP-6 decision 1                                       | **yes** (roadmap WS5)               |
| Workstream 6 (browser-hosting switch)                          | shipped             | B5-3 #1542, B6-7 #1585                                        | no           | —                                                                              | no                                  |
| Workstream 7 (upgrade/rollback)                                | shipped             | B6-8 #1590                                                    | no           | —                                                                              | no                                  |
| Workstream 8 (250/100/25 measurements)                         | shipped             | B6-9 #1592                                                    | no           | —                                                                              | no                                  |
| Workstream 9 (operational load)                                | shipped             | B6-10 #1601                                                   | no           | —                                                                              | no                                  |
| Workstream 10 (failure/restore drills)                         | shipped             | B6-11 #1603, close-out #1613                                  | no           | —                                                                              | no                                  |
| Workstream 11 (supply-chain signing)                           | partial             | B6-12 #1617, follow-up #1618 (tag rehearsal owed)             | no           | RC tag run id                                                                  | no — final pass                     |
| Workstream 12 (operator docs)                                  | shipped             | B6-13 #1623, `ccd9a5dc`                                       | no           | —                                                                              | **yes**                             |
| Workstream 13 (ARM64 must-close defects)                       | refuted as open     | OC-0320/0339/0344 #1530 `1edd777f`; OC-0332 #1532 `39b2423f`  | **yes**      | amended in place, satisfied precondition                                       | **yes**                             |
| Workstream 14 (voice load harness gap)                         | refuted             | B6-9 #1592, `Server/scripts/voice-load.sh`                    | **yes**      | amended in place, closed                                                       | **yes**                             |
| Workstream 15 (R-09 tag rehearsal)                             | pending             | B6-12 #1617 (tag run not yet rehearsed — Acceptance unticked) | no           | rehearsed by B6-12, tag `<tag>`, run `<id>`                                    | no — B6-12 Task 6 not complete      |
| Workstream 16 (`server-info` endpoint)                         | shipped             | B6-7 #1585                                                    | no           | —                                                                              | no                                  |
| Workstream 17 (docs carryover)                                 | shipped             | B6-14 #1620, `d9880bb6`                                       | no           | —                                                                              | **yes**                             |
| Workstream 18 (BPR-053/trust-model)                            | shipped             | B6-15 #1622, `85c97507`                                       | no           | B6-15's landing sentence, copied verbatim                                      | **yes**                             |
| OC-0320                                                        | fixed               | #1530, `1edd777f`                                             | **yes**      | `**Fixed**` prefix                                                             | **yes**                             |
| OC-0331                                                        | fixed               | #1530, `1edd777f`                                             | **yes**      | `**Fixed**` prefix                                                             | **yes**                             |
| OC-0332                                                        | fixed               | #1532, `39b2423f`                                             | **yes**      | `**Fixed**` prefix                                                             | **yes**                             |
| OC-0339                                                        | fixed               | #1530, `1edd777f`                                             | **yes**      | `**Fixed**` prefix                                                             | **yes**                             |
| OC-0344                                                        | fixed               | #1530, `1edd777f`                                             | **yes**      | `**Fixed**` prefix                                                             | **yes**                             |
| OC-0346                                                        | fixed               | #1454, `775eba50`                                             | no           | already carries B3-9's `**Fixed**` prefix                                      | no — already closed                 |
| OC-0350                                                        | fixed               | #1530, `1edd777f`                                             | **yes**      | `**Fixed**` prefix                                                             | **yes**                             |
| OC-0353                                                        | fixed               | #1530, `1edd777f`                                             | **yes**      | `**Fixed**` prefix, rehearsal clause replaced by the fix's own test            | **yes**                             |
| OC-0355                                                        | fixed               | #1530, `1edd777f`                                             | **yes**      | `**Fixed**` prefix                                                             | **yes**                             |
| OC-0361                                                        | fixed               | #1530, `1edd777f`                                             | **yes**      | `**Fixed**` prefix                                                             | **yes**                             |
| OC-0364                                                        | fixed               | #1530, `1edd777f`                                             | **yes**      | `**Fixed**` prefix                                                             | **yes**                             |
| OC-0367                                                        | fixed               | #1530, `1edd777f`                                             | **yes**      | `**Fixed**` prefix                                                             | **yes**                             |
| OC-0373                                                        | fixed               | #1530, `1edd777f`                                             | **yes**      | `**Fixed**` prefix                                                             | **yes**                             |
| S-07 (benchmarks)                                              | partial             | B3-6 #1459 (benchmarks); B6-9 #1592 (load baselines)          | **yes**      | closure note naming both PRs                                                   | **yes**                             |
| S-14 (published capacity result)                               | resolved/superseded | B6-9 #1592, `26cd9675`                                        | **yes**      | `resolved/superseded`, run ids                                                 | **yes**                             |
| S-17 (dependency scanning)                                     | pending             | unchanged unless B6-12 adds scanning                          | no           | —                                                                              | no                                  |
| R-04 (immutable digests)                                       | pending             | after B6-12                                                   | no           | B6-12's landing sentence                                                       | no — final pass                     |
| R-07 (SBOM/provenance)                                         | pending             | after B6-12                                                   | no           | B6-12's landing sentence, or `**Partial:**`                                    | no — final pass                     |
| R-09 (exact-SHA gate rehearsal)                                | pending             | after B6-12                                                   | no           | phase `B1/B6/B10`, rehearsal sentence                                          | no — final pass                     |
| BG-06 (TLS deferral)                                           | pending             | #1619's territory                                             | no           | deferred sentence citing HP-6 decision 1                                       | no — final pass, and #1619          |
| BG-15 (support-bundle landing)                                 | shipped             | #1573, `3221fe9e` (2026-09-10)                                | **yes**      | landed sentence with the PR that added it                                      | no — held for final pass (see note) |
| BG-20 (ARM64 assets/multi-arch)                                | pending             | needs RC tag run id                                           | no           | partial/shipped with tag run id                                                | no — final pass                     |
| BPR-011                                                        | pending             | needs RC tag run id                                           | no           | satisfied sentence                                                             | no — final pass                     |
| BPR-012                                                        | pending             | needs HP-6 date                                               | no           | first-evidence-at-HP-6 sentence                                                | no — final pass                     |
| BPR-013                                                        | n/a                 | already carries its landed shape                              | no           | —                                                                              | no                                  |
| BPR-014                                                        | pending             | already reads "Blocked on B6-3"                               | no           | re-read against the HP-6 decision                                              | no — final pass                     |
| BPR-015                                                        | pending             | needs HP-6 decision date                                      | no           | blocked sentence, BPR-014's shape                                              | no — final pass                     |
| BPR-016                                                        | pending             | needs HP-6 decision date                                      | no           | blocked sentence, BPR-014's shape                                              | no — final pass                     |
| BPR-030                                                        | pending             | B6-9 #1592 landed; B6-10 rows still owed                      | no           | satisfied sentence                                                             | no — final pass                     |
...
Note on BG-15: `git log -S'support-bundles' --oneline -- Server/admin/` finds
exactly one commit, `3221fe9e` ("feat: add reliability foundations and
recovery diagnostics", PR #1573, merged 2026-09-10), which is an ancestor of
`dev`. `/admin/api/support-bundles/{preview,download}` (`Server/admin/api.go:237-238`)
are unambiguously landed by that PR. This is stale today — the row could go
in this pass — but PR #1573's title does not name B6 and the register's
closure-wording template for BG-15 also names the HP-6 operator record as
corroborating evidence, which does not exist yet; held for the final pass
rather than writing a landed sentence that leans on evidence this pass cannot
cite. Flagged here so the final pass does not have to re-run the search.
...
| -------- | -------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------- | ---------------------------------------------------------------------------------------------------------------------------- | ---------- |
| B6-1     | Standalone release assets                                | An owner downloads a Windows x64/ARM64 executable or Linux x64/ARM64 archive that starts, migrates, becomes healthy, drains and restarts. **The build and per-asset lifecycle smokes are in place (B6-1 #1580); three acceptance rows await the first tag run**, so the milestone stays `in-progress` until the beta tag proves them.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | in-progress | [b6-1-standalone-release-assets.plan.md](../../.claude/plans/b6-1-standalone-release-assets.plan.md)                         | 1          |
...
| B6-10    | Operational performance measurements                     | Reconnect storms, database-pool wait deltas, recipient-receipt fan-out latency, voice control, upload admission through quota and storage, TLS overhead and graceful shutdown are all measured and published. **Measurements published** in [capacity.md](../capacity.md), 'The operational profiles': four runs at `e57335c7` on the constrained cgroup. Open findings: OC-0445 (send-acknowledgement and delivery p95 miss their budgets under the operational profile), OC-0446 (the restart drill's pre/post windows are not comparable), OC-0447 (the ceiling search is bounded by the per-channel topic limiter from step 200) — **OC-0446 and OC-0447 were fixed 2026-09-23**; OC-0445 was a phase-locked load generator after the reconnect storm and is fixed in the harness, with the server's per-hop burst cost recorded as OC-0454 (low, open). The register carries the live counts. | complete    | [b6-10-operational-performance-measurements.plan.md](../../.claude/plans/b6-10-operational-performance-measurements.plan.md) | 9          |
| B6-11    | Failure and recovery drills                              | Backup/restore, deletion-marker restore, disk-full, low-headroom, corrupt input, unhealthy dependency, interrupted migration and rollback all pass, with byte-level erasure evidence under active readers. **The restart-under-load drill already exists** as `load-baseline.yml` `profile=restart` (B6-10): reuse it for the unhealthy restart cases rather than planning a second one                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | complete    | [b6-11-failure-recovery-drills.plan.md](../../.claude/plans/b6-11-failure-recovery-drills.plan.md)                           | 10         |
| B6-12    | Signed, traceable release inputs and outputs             | Containers and build inputs are pinned or auto-reviewed; SBOM, provenance, checksums and a source snapshot are signed; one tag is rehearsed against `gate-evidence` and `environment: release`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | in-progress | [b6-12-signed-traceable-release.plan.md](../../.claude/plans/b6-12-signed-traceable-release.plan.md)                         | 11, 15     |
| B6-13    | Operator documentation                                   | Local logs, support-bundle generation, capacity limits, ports, storage growth, certificate trust, recovery, updates and safe failure are all documented for a stranger. **Written 2026-09-18**; _unverified_: where stdout lands on a real systemd, NSSM, Docker and Task Scheduler host — read from each supervisor's configuration and documentation, not run; the HP-6 operator record is where it gets exercised                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | complete    | [b6-13-operator-documentation.plan.md](../../.claude/plans/b6-13-operator-documentation.plan.md)                             | 12         |
| B6-14    | Service-boundary handle reconciliation                   | Every direct database-handle use, not only imports, has a named service, adapter or transaction-boundary owner, and the guard rejects unclassified access — with replay locking and persister ordering preserved                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | complete    | [b6-14-service-boundary-handles.plan.md](../../.claude/plans/b6-14-service-boundary-handles.plan.md)                         | 17         |
| B6-15    | Privacy-claim reconciliation                             | BPR-053, requirement traceability and privacy acceptance evidence match the HP-4-approved retained audit-token design and `docs/trust-model.md`, with correlation and key-holder limits recorded consistently                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | complete    | [b6-15-privacy-claim-reconciliation.plan.md](../../.claude/plans/b6-15-privacy-claim-reconciliation.plan.md)                 | 18         |
| **HP-6** | **Operator and capacity acceptance — the owner signs**   | An owner unfamiliar with the code deploys each mode from current documentation, understands the network and trust limits, recovers a backup, rotates trust and interprets failure. **2026-09-18:** the operator-usability record can now be attempted against deployment.md's headings; the support-bundle section was written from source (`Server/admin/support_bundle.go`, `docs/architecture/diagnostics.md`), not a live download; the certificate-expiry behaviour is measured, by the Go test `Server/auth/tls_expiry_test.go` plus `Client/src-tauri/src/tofu.rs` read from source                                                                                                                                                                                                                                                                                                         | pending     | —                                                                                                                            | HP-6       |
| B6-16    | Register and roadmap reconciliation                      | The issue register and roadmap match what B6 actually shipped                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | in-progress | [b6-16-register-roadmap-reconciliation.plan.md](../../.claude/plans/b6-16-register-roadmap-reconciliation.plan.md)           | exit       |
...
- [x] **Decided 2026-09-18:** B6-11 is closed on the owner's instruction. Ten of
      its eleven acceptance rows are evidenced — the six `go test` drills pass
      at `ee5e287f`, and the process-level phases are the local linux/amd64 run
      of 2026-09-16 — and the eleventh cannot be yet: `upgrade-rehearsal.yml`
      is on `dev` and not on `main`, so a dispatch resolves nothing and the
      schedule never fires. No GitHub-runner run of the drills exists. That
      evidence is owed by B6-12 Task 6, the step that puts `dev` on `main`:
      one dispatched run, its id recorded in the B6-11 plan and in
      [data-lifecycle.md](../architecture/data-lifecycle.md), which also
      settles phase D's container leg and the paced disk-full re-measurement.
...
_Status: reconciled at B6-16 (2026-09-25). Requirements and implementation are
complete except the first beta tag run that proves B6-1/B6-2/B6-9/B6-12 and the
B6-1 ARM64 acceptance rows, and HP-6, which the owner deferred to run against
that tag (D-02). The B6-3 – B6-5 TLS block is the accepted limitation D-01 and
moves to B11._
