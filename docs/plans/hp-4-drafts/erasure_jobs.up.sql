-- B4-9 draft: complete account erasure (BPR-052, BG-11).
-- The database half of an erasure is one transaction (data-lifecycle O1
-- A1); the file half (attachment blobs, the avatar) cannot be, so it is
-- journaled here and resumed at startup until every file is gone. The row
-- outlives the subject: user_id is a bare integer, not a foreign key, because
-- the users row is hard-deleted by the same transaction that writes it.
