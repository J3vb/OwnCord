...
| T-2026-07-25-19 | LOW | No coverage threshold or ratchet on the Go side; no coverage instrumentation for Rust at all. The client's 70% vitest threshold is the only enforced floor anywhere | **RESOLVED** (B3-6, 2026-08-30) — `Server/coverage-floor.json` is the baseline store this note said the repo lacked: an aggregate floor plus per-package floors for the five core packages, enforced by `Server/scripts/coverage-floor.sh` on the ubuntu leg of Server Build & Test, ratchet rule in `Server/CLAUDE.md`. Rust still has no coverage instrumentation |
...
`UserProfilePopup.ts` 55.2% *branches*.
...
| 2 | Repair the e2e login helper so the web suite passes, then promote `client-e2e` to blocking | T-…-21, T-…-06 | HIGH |
| 3 | Fix the `drag-reorder.ts` ref-count asymmetry and update its pinning test | §4 | MEDIUM |
| 4 | Migrate the seven `HandleLiveKitHealthForTest` callers to `LiveKitHealthHandlerForTest` and delete the duplicated hook | T-…-11 | LOW |
