...
added the two camera commands, and phase 3 (branch
...
| Distinct `invoke` command names called from `Client/src/`  | 45    |
...
The handler count covers both attribute spellings — 39 `#[tauri::command]` plus
12 `#[tauri::command(async)]` — so a `git grep '#\[tauri::command\]'` with exact
brackets undercounts to 39. Attributes and registrations are two different
counts: of the 51 attributed functions, 49 appear in `generate_handler!`
(`Client/src-tauri/src/lib.rs`); `open_devtools` sits behind
`#[cfg(feature = "devtools")]` and the sixteen `native_voice_*` commands behind
`#[cfg(target_os = "linux")]`, so a default build registers 48 on Linux and 32
