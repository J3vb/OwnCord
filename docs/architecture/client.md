...
and within one page (cycles 6 and 9, after that page's reconnect). Nodes also
pass on a net move of at most 2, because a detached node can be in flux at one
sample even after the settle loop (a 1-node move in a two-sample page series is
a slope of 1/3); a real leak accumulates on every read, so it still fails. Heap
