...
| Axis | Model                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| ---- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| A1   | Backup: an interrupted `VACUUM INTO` is removed; a kill between `VACUUM INTO` and `integrity_check` leaves a file the next list shows and the integrity check never ran on — restore re-checks it before use, so it cannot be restored if broken. Restore: a kill **before** the close leaves the live file intact (the restart serves the old data); a kill **during** `copyFile` — a signal, not an error, so the rollback branch does not run — leaves a truncated live database. The declared recovery is manual: the `pre_restore_*` safety copy is on disk and `db.Open` refuses the truncated file on restart, so the operator restores the safety copy by hand. There is no automatic repair for this window. Both halves are measured, not asserted: `TestB611_CorruptFilesFailClosed` (`Server/db/b6_11_corrupt_drills_test.go`) truncates a copy of the alpha snapshot to half and the boot stops at `db.Open` — `pinging sqlite db: database disk image is malformed (11)`, before `MigrateFS` runs at all. The same drill measures what that refusal does **not** cover, which is the limit of the sentence above: a database whose _content_ pages are damaged opens and migrates successfully, because `Open`, `MigrateFS` and the pragmas read the schema and the tracker and never a table page — the boot stops only when something reads the damaged table, which on a real install is the first request that touches it. `integrity_check` on the same file does report the damage, and the server runs no such check at boot: a boot-time check is an open owner question (B6-11 plan, question 2), not a silent gap.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
...
| 8c  | Message retry receipts          | `message_delivery_receipts` (`client_message_id`, request SHA-256, original message id/timestamp, expiry)                                                                                                                                    | `user_id`; request fingerprint                                                                             | Deleted in the account-erasure transaction. Retained across message/channel deletion until the embedded logical ID expires (24 hours), then pruned by the maintenance sweep. No message content is stored.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | CASCADE on sender                 | Implemented (reliability foundations). Expired logical IDs are rejected even after pruning; missing/deleted originals are refused, never recreated.                                                                                              |
...
| 12a | Upload byte counter             | `user_storage` (`user_id`, `bytes_used`) (migration 044, B5-2)                                                                                                                                                                               | `user_id`                                                                                                  | Row deleted in the transaction (`erasureStatements`, and `ON DELETE CASCADE` behind it)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | user_id: CASCADE                  | Done (B5-2). B4-11: the maintenance recount lowers the counter to the rows that survive the sweep, which is the decrement S3-a owes                                                                                                              |
...
| 25  | Free pages and WAL              | the SQLite file's free list and `-wal`                                                                                                                                                                                                       | content of updated/deleted rows                                                                            | The erasure connection runs `PRAGMA secure_delete = ON` for its transaction (freed content zeroed) and `wal_checkpoint(TRUNCATE)` after commit; other writes keep the default. **The truncating checkpoint can be refused**: a reader holding a read transaction makes it return `busy=1`, and the log then keeps the erased frames — the writer's autocheckpoint backfills them into the main file and never truncates, so `busy = 0` with `log == checkpointed` is not evidence that the log is empty. Since B6-11 the erasure records that it owes a truncate (`db.checkpointOwed`) and one is run unconditionally — after both start-up replays, and at the end of every erasure-service tick — until the log is actually truncated. The main database file holds nothing at every point of every shape. | —                                 | Done (B4-9, HP-4 decision 2); the retained-bytes shapes are B6-11's measurement, tabulated below under "B6-11 — the failure and recovery drills". `TestB611_ErasureBytes` (E1–E4, `Server/db/b6_11_drills_test.go`)                              |
...
### B6-11 — the failure and recovery drills (2026-09-16)

Nine drills, one per failure axis the roadmap names, all on copies of the same
snapshot. Two instruments: `go test` drills beside the HP-4 ones, and
`cmd/smoke -drills`, which boots the real binary and kills it, fills its
filesystem and kills its SFU. The rule the instrument enforces is that **a
phase never prints a `pass` it did not run**: a phase that cannot measure
prints `skipped`, a single step that cannot run turns the phase's verdict into
`passed, except step N`, and both are repeated on the run's last line. Every
row below names the test or the phase that produced it; the process-level rows
are from the local linux/amd64 run of 2026-09-16 (`upgrade-rehearsal.yml` runs
the same legs nightly — its run id is in the pull request).

| #   | Drill                   | Instrument                                                                                  | Result                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| --- | ----------------------- | ------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | Backup and restore      | phase R steps 1–5                                                                           | Pass. 20 sockets open and one reader holding a read transaction through the real admin endpoints: the restart broadcast arrives, the boot exits 0, `pre_restore_*` is present and integrity-checks, the restored data equals the backup, and the account registered after the backup is gone.                                                                                                                                                                                                              |
| 2   | Deletion-marker restore | phase R steps 6–7                                                                           | Pass. An account erased after the backup is erased again before the server serves — both in the restored database and on disk, with the `account_erasure_replayed` audit row. Both variants measured: with the marker file gone the install boots and the erased account is back (decided behaviour — the plan's question 1 — recorded, not asserted as a failure); with `erasure.key` gone the boot **fails closed** and names the reason.                                                                |
| 3   | Disk full               | phase D (`dFull`)                                                                           | Pass. 50 sends against a full filesystem: 50 answered with an error frame, 0 unanswered, no exit; `/health` keeps answering `503 degraded/disk`; the log names the full-disk error; after the space is given back chat, upload and `/health` recover **without a restart**, and the rebooted database holds exactly the acknowledged messages. No silent path was found.                                                                                                                                   |
| 4   | Low headroom            | phase D (`dHeadroom`, `dBackup`)                                                            | Pass. Crossing `server.min_free_disk_mb`: `/health` `503 degraded/disk`, an upload `507 STORAGE_LOW_DISK`, a message still accepted (the floor is the upload path's), and a backup taken under the floor returns a file `integrity_check` accepts — the OC-0212 invariant (no partial left behind) rather than a predicted error.                                                                                                                                                                          |
| 5   | Corrupt input           | `TestB611_CorruptFilesFailClosed`, phase C                                                  | Pass, with the limitations below. Truncated database: refused at `db.Open`. Corrupt marker row: refused at the stage's read (the store's _open_ reports success). Flipped page: **not** caught by the boot. A corrupt backup is refused with 400 and the live database is untouched; a `config.yaml` with a syntax error is refused before anything is created, naming file and line. A corrupt backup file's own path is covered by `TestCheckBackupIntegrity_ValidAndCorrupt` and cited, not re-drilled. |
| 6   | Unhealthy dependency    | phase S                                                                                     | Pass, with a finding held privately. The supervised SFU killed with `-9`: `voice_join` fails closed with `VOICE_ERROR`, the diagnostics endpoint reports `livekit_health=false`, the supervisor restarts it, and a token is minted again.                                                                                                                                                                                                                                                                  |
| 7   | Interrupted migration   | `TestB611_InterruptedMigrationRollsBack`                                                    | Pass. The crash image carries no tracker row and the pre-transaction schema; the next open re-applies the migration **exactly once** and reaches a schema byte-identical to the uninterrupted HEAD schema, `integrity_check` `ok`.                                                                                                                                                                                                                                                                         |
| 8   | Rollback                | `TestB611_InterruptedRollbackLeavesSchema`, `TestMigrationRollbackRehearsalOnAlphaSnapshot` | Pass. A reversal interrupted after a statement boundary leaves `sqlite_master` and `schema_versions` byte-identical (the `-bail`-inside-`BEGIN` rule, asserted rather than described); the full reversal list from that same database still lands on the snapshot's schema.                                                                                                                                                                                                                                |
| 9   | Byte-level erasure      | `TestB611_ErasureBytes` E1–E4                                                               | Pass. See the table below.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |

**E1–E4, the byte-level erasure.** One sentinel is planted in 200 messages, in
the FTS index and in an upload's bytes, with the writer's autocheckpoint off so
the planted rows are WAL frames when the erasure runs; eight roots are then
counted for it — the database, its `-wal` and `-shm`, the uploads directory,
the backup directory, and the marker file with its sidecars — at every scan
point, with a positive control that fails if the sentinel was not readable in
the `-wal` and the upload first.

| Scenario                                 | What the erasure's truncate meets                               | Main file | `-wal` after the erasure                                                                                                                                                                                     | What removes the bytes                                                            |
| ---------------------------------------- | --------------------------------------------------------------- | --------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------- |
| E1 idle                                  | nothing                                                         | 0         | 0 bytes — the truncate ran                                                                                                                                                                                   | the erasure's own checkpoint (this is the unconditional shape)                    |
| E2 a reader holds a transaction          | `busy=1`; the erasure logs that the next checkpoint finishes it | 0         | still full length and still holding the sentinel at **all three** scans, the third of which is that "next checkpoint" — it completed (`busy=0`, `log == checkpointed`) and **backfilled without truncating** | the tick retry, or the start-up pass                                              |
| E3 the checkpoint cannot run             | `busy=1`                                                        | 0         | full length while the reader holds; 0 after the last close                                                                                                                                                   | the last close (checkpoint + `-wal` delete) — or the retry, whichever comes first |
| E4 a crash between commit and checkpoint | nothing ran                                                     | 0         | full length in the crash image **and** after the whole recovery path (open → migrate → marker replay → resume), which never checkpointed                                                                     | the start-up pass, which runs unconditionally after both replays                  |

The main database file is 0 at every point of all four scenarios, and E4's
first scan shows why the truncate is a hygiene question rather than a
...
- **No boot-time integrity check** (drill 5, and O4 A1 above): `Open`,
  `MigrateFS` and the marker store's open all report success on a database or a
  marker file whose _content_ is damaged; something has to read the damaged
  table first. Owner question 2, and the reason `db.Open` refusing a truncated
  file is a narrower guarantee than it sounds.
- **`-shm` cannot be part of a crash image on Windows** (drill 7): a live
  connection holds region locks on it, so the read fails. It is a derived index
  SQLite rebuilds from the WAL, and the image is the database and its log.
- **Drill 7 does not exercise WAL recovery of uncommitted frames on this
  build.** `PRAGMA cache_size=1` plus `cache_spill` do not spill two DDL
  statements' dirty set — the fixture's `-wal` is byte-identical before and
  after the interrupted transaction — so the crash image is the pre-transaction
  database rather than one holding uncommitted frames. The spilling path itself
  works: a ~2 MB uncommitted transaction added to the same connection took the
  `-wal` from 902,312 to 3,658,592 bytes. The assertions hold for either image;
  only the uncommitted-frames case is untested.
- **The container leg of phase D measured nothing on the development machine.**
  Docker Desktop's containerd snapshotter resolves `docker cp` into the layer
  _below_ the mounted tmpfs, so the copy succeeded outside the filesystem the
  server writes to; the harness's own reachability probe reports that as
  `skipped` rather than a pass. The dispatched `upgrade-rehearsal.yml` run (a
  runner with the classic graph driver) is what settles it.

**A finding held privately.** One of the nine recorded a finding in shipped
behaviour that this branch does not fix. Per "The private half" it is not
described here — its row above names the drill and the verdict and stops there —
and it is not a findings-ledger row: the ledger records a finding once it is
resolved, and this one is not resolved. It goes to the owner from the plan
workspace and the pull request.

The other rows whose behaviour is _not_ fixed are decided behaviour, not findings
held back: a missing marker file leaving an install with no erasure history is
what the plan's question 1 settles on (boot, and log an error naming it), and it
is written out for the operator in
[deployment.md](../deployment.md#restore) because that is the page a backup
procedure is read from.
...
-- 8c message retry receipts (migration 051; deleted with their sender)
SELECT COUNT(*) FROM message_delivery_receipts WHERE user_id = :uid;
...
-- 12a upload byte counter (migration 044, B5-2)
SELECT COUNT(*) FROM user_storage WHERE user_id = :uid;
