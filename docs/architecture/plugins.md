...
| Upgraded install           | off     | an older `config.yaml` has no `plugins` key; the loader starts from the compiled defaults, so the absent key reads as `false`. A misspelled key is warned about and ignored (`Server/config/config.go:666`), which also leaves the default `false` in force |
| Docker                     | off     | image built without the tag (`Dockerfile:13`); `OWNCORD_PLUGINS_ENABLED=true` (`Server/config/env.go` envOverrides, called from `config.go:721`) would flip the flag but the binary still cannot execute a module                                           |
...
| Manifest      | JSON `plugin.json`. Entrypoint must be a relative `.wasm` path.                                                                                                                                                                                                                                                                                                                                                                                                                                      | `manifest.go:52-61`, `:115`, `:144-148`; `loader.go:39`                                                          |
...
| CPU           | a wall-clock deadline per guest call, `list_commands` included — `plugins.cpu_budget_ms` is the ceiling (100 ms when unset), and a manifest's `cpu_budget_ms` can lower it but never raise it (the example's 50 ms stays 50 ms; a manifest asking for 5 s gets the ceiling). Not fuel-based: a runaway loop is killed by closing the module, and the next dispatch re-instantiates from the retained compile. A result region over 64 KiB is refused                                                 | `sandbox_wazero.go` `guestBudget`, `maxGuestResultBytes`; `config.go:180`                                        |
