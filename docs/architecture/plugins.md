...
| Upgraded install           | off     | an older `config.yaml` has no `plugins` key; the loader starts from the compiled defaults, so the absent key reads as `false`. A misspelled key is warned about and ignored (`Server/config/config.go:520`), which also leaves the default `false` in force |
