...
| `adapter`   | a transport adapter that uses `db` types or pure helpers only — response shapes, status helpers — no persistence call |   39 |
...
no query of its own. The table is down from 59 rows to 59: B3-8's move-outs took it well below today's count before four adapter rows landed back — `api/report_handler.go` and `api/moderation_queue_handler.go` (B5-8), `api/moderation_handler.go` (B5-9), and `api/appeal_handler.go` (B5-10) — and every row left
...
may read. 38 of the 59 rows are type-only, 30 of them `adapter`.
...
| `internal/app/maintenance.go`     | `DB×3`                                                                                                                                                          | —                                                                            | `CleanupExpiredSecondFactorState` `DeleteExpiredSessions` `DeleteOrphanedAttachments` `RetireModerationActions`                    | calls     | boundary    | —      | periodic worker: expired sessions, backups, orphan attachments                                                                                                                                |
...
59 files import `db` outside `db/` and `service/` (. 1, admin 12, api 13, auth 2, cmd/gendocs 1, cmd/seed 2, internal/app 7, plugin 1, ws 20); 38 are type-only; 0 unlisted.
Dispositions: adapter 39, boundary 20. Move targets: .

<!-- dbinventory:end -->

Reading the table:

- **Type-only rows (38, of which 30 are `adapter`)** mostly need no service:
