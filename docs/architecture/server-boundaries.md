...
no query of its own. The table is down from 59 rows to 53, and every row left
...
may read. 34 of the 53 rows are type-only, 26 of them `adapter`.
...
| `api/upload_handler.go`           | `Role` `User×3`                                                                                                                              | —                                                         | —                                                                                                                                  | type-only | adapter     | —      | AttachmentAccess/User/Role types while serving the bytes; UploadService owns the access decisions                                                                                             |
...
53 files import `db` outside `db/` and `service/` (. 1, admin 11, api 9, auth 2, cmd/gendocs 1, cmd/seed 2, internal/app 6, plugin 1, ws 20); 34 are type-only; 0 unlisted.
Dispositions: adapter 35, boundary 18. Move targets: .
