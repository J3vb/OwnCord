...
| `boundary`  | an explicit composition or transaction boundary that legitimately owns a handle (process entry, CLIs, health probe)   |   10 |
...
| `api/router.go`                   | `DB×4`                                                                                                                       | —                                                         | `PingRead` `SQLDb`                                                                                                                                                                                                                                                                     | calls     | boundary    | —            | health probe (PingRead, SQLDb); hub construction leaves in B3-3    |
...
53 files import `db` outside `db/` and `service/` (. 1, admin 16, api 10, auth 2, cmd/gendocs 1, cmd/seed 1, internal/app 4, plugin 1, ws 17); 15 are type-only; 0 unlisted.
Dispositions: adapter 17, boundary 10, move 26. Move targets: auth 7, channel 6, connection 1, role 1, settings-ops 4, upload 2, user 2, voice 3.
