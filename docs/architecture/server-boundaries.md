...
no query of its own. B5-7 then added one row back — `api/nsfw_handler.go`,
type-only `adapter` (`db.User` on the context key; `NSFWService` owns every
call) — so the table is down from 59 rows to 58, and every row left is an
`adapter` (db types, no persistence call) or a `boundary` that owns a
