...
| `adapter`   | a transport adapter that uses `db` types or pure helpers only — response shapes, status helpers — no persistence call |   38 |
...
no query of its own. The table is down from 59 rows to 58 (B5-9 added one row back, `api/moderation_handler.go`, an adapter), and every row left
...
may read. 37 of the 58 rows are type-only, 29 of them `adapter`.
...
58 files import `db` outside `db/` and `service/` (. 1, admin 12, api 12, auth 2, cmd/gendocs 1, cmd/seed 2, internal/app 7, plugin 1, ws 20); 37 are type-only; 0 unlisted.
Dispositions: adapter 38, boundary 20. Move targets: .

<!-- dbinventory:end -->

Reading the table:

- **Type-only rows (37, of which 29 are `adapter`)** mostly need no service:
