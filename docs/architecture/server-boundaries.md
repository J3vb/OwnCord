...
| ----------- | --------------------------------------------------------------------------------------------------------------------- | ---: |
| `move`      | persistence or a domain decision that belongs behind a service; **Family** names the service B3-8 (or B3-2) builds    |   12 |
| `adapter`   | a transport adapter that uses `db` types or pure helpers only — response shapes, status helpers — no persistence call |   30 |
...
entirely (15 → 14) — the first file to leave the table since B3-2 — and the
user family (14 → 12), which took the admin panel's user reads behind
`UserService` and the connection teardown's one write behind a seam.
...
59 files import `db` outside `db/` and `service/` (. 1, admin 14, api 10, auth 2, cmd/gendocs 1, cmd/seed 2, internal/app 6, plugin 1, ws 22); 32 are type-only; 0 unlisted.
Dispositions: adapter 30, boundary 17, move 12. Move targets: auth 7, connection 2, voice 3.
