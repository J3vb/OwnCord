...
| ----------- | --------------------------------------------------------------------------------------------------------------------- | ---: |
| `move`      | persistence or a domain decision that belongs behind a service; **Family** names the service B3-8 (or B3-2) builds    |    9 |
...
changes family rather than disposition, and the connection family (10 → 9),
after which **every remaining `move` row belongs to one family, `auth`**.
...
59 files import `db` outside `db/` and `service/` (. 1, admin 14, api 10, auth 2, cmd/gendocs 1, cmd/seed 2, internal/app 6, plugin 1, ws 22); 34 are type-only; 0 unlisted.
Dispositions: adapter 33, boundary 17, move 9. Move targets: auth 9.
