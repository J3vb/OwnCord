...
| ----------- | --------------------------------------------------------------------------------------------------------------------- | ---: |
| `move`      | persistence or a domain decision that belongs behind a service; **Family** names the service B3-8 (or B3-2) builds    |    7 |
| `adapter`   | a transport adapter that uses `db` types or pure helpers only — response shapes, status helpers — no persistence call |   33 |
| `boundary`  | an explicit composition or transaction boundary that legitimately owns a handle (process entry, CLIs, health probe)   |   18 |
...
after which **every remaining `move` row belongs to one family, `auth`**, and
the auth family's token sub-family (9 → 7), whose two rows go opposite ways:
`admin/handlers_tokens.go` is **deleted** — it stopped importing `db` at all,
the second file to leave the table — while `token_cli.go` becomes `boundary`,
because a bootstrap CLI legitimately opens and migrates its own handle even
once it makes no query of its own.
...
may read. 34 of the 59 rows are type-only, 25 of them `adapter`.

Type-only is a shape, not a verdict, and the table keeps the two apart: one
`move` row (`admin/middleware.go`) is type-only as well, because what still
has to move out of it is a decision it takes on an already-loaded row rather
than a query it issues. Reading the shape column as the disposition would
call it done.

Two seams in `ws/readers.go` are the same shape with a difference worth
naming: unlike the read seams, `*db.DB` does **not** satisfy `VoiceStore` or
`PresenceStamper`. Their methods are `service.VoiceService`'s and
`service.UserService`'s, so the decisions behind them — which insert a
capacity limit selects, which channel a compensating write may touch, which
saved status survives a reconnect — cannot be answered by the handle alone.
That is the line between a seam that narrows the handle and a family that has
moved, and it is why `HubReaders` (which `DBReaders` wires wholesale) now
holds only the four handle-backed read seams.
...
| `ws/hub_sweep.go`                 | `User`                                                                                                                                       | —                                                         | `GetSessionsWithBanStatusBatch`                                                                                                    | calls     | move        | auth   | session sweep re-reads the batched session/ban rows; the voice halves left with the voice family                                                                         |
...
| `ws/readers.go`                   | `Channel×3` `ChannelOverride×2` `ChannelUnread` `DB` `DMChannelInfo` `MemberSummary` `Role×4` `SessionWithBanStatus` `User×3` `VoiceState×5` | —                                                         | —                                                                                                                                  | type-only | adapter     | —      | the hub's read seams plus the service-backed VoiceStore and PresenceStamper: db types in the interface signatures, and DBReaders wiring the handle behind the read seams |
| `ws/replay.go`                    | `PersistedEvent`                                                                                                                             | —                                                         | —                                                                                                                                  | type-only | adapter     | —      | PersistedEvent type in the cold-tier filter; the resume path's reads bind the VisibilityReader seam and its status stamp goes through PresenceStamper                    |
| `ws/serve.go`                     | `DB`                                                                                                                                         | —                                                         | `GetRoleByID` `GetUserByID`                                                                                                        | calls     | move        | auth   | the handle ServeWS threads to upgradeAndAuth; the connect/disconnect status stamps left with the connection family                                                       |
| `ws/serve_auth.go`                | `DB×2` `User`                                                                                                                                | `StatusOffline` `WriteAudit()`                            | `GetRoleByID` `GetSessionByTokenHash` `GetUserByID`                                                                                | calls     | move        | auth   | handshake auth: session, user and role lookups, connect audit, failed-handshake teardown                                                                                 |
...
| `ws/serve_ready.go`               | `Channel×10` `ChannelOverride×6` `ChannelUnread×2` `DMChannelInfo×4` `MemberSummary×3` `Role×4` `User` `VoiceState×5`                        | `StatusOffline×3`                                         | —                                                                                                                                  | calls     | adapter     | —      | ready snapshot reads through ReadySnapshotReader; fresh-connect stale-voice cleanup through VoiceService                                                                 |
...
58 files import `db` outside `db/` and `service/` (. 1, admin 13, api 10, auth 2, cmd/gendocs 1, cmd/seed 2, internal/app 6, plugin 1, ws 22); 34 are type-only; 0 unlisted.
Dispositions: adapter 33, boundary 18, move 7. Move targets: auth 7.

<!-- dbinventory:end -->

Reading the table:

- **Type-only rows (32, of which 22 are `adapter`)** mostly need no service:
