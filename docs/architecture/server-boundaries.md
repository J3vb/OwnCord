...
| `adapter`   | a transport adapter that uses `db` types or pure helpers only — response shapes, status helpers — no persistence call |   40 |
...
context only; ReportService owns every call) — so the table is down from 59 rows to 60, and every row left is an `adapter` (db types, no persistence
...
may read. 39 of the 60 rows are type-only, 31 of them `adapter`.
...
60 files import `db` outside `db/` and `service/` (. 1, admin 12, api 14, auth 2, cmd/gendocs 1, cmd/seed 2, internal/app 7, plugin 1, ws 20); 39 are type-only; 0 unlisted.
Dispositions: adapter 40, boundary 20. Move targets: .

<!-- dbinventory:end -->

Reading the table:

- **Type-only rows (39, of which 31 are `adapter`)** mostly need no service:
