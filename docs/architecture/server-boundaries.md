...
| ----------- | --------------------------------------------------------------------------------------------------------------------- | ---: |
| `move`      | persistence or a domain decision that belongs behind a service; **Family** names the service B3-8 (or B3-2) builds    |   16 |
...
(28 → 24), the channel family's three parts (24 → 17), and the role
family, whose single row is **deleted** rather than downgraded because
`admin/handlers_roles.go` stopped importing `db` entirely (17 → 16, and
the first file to leave the table since B3-2).
...
59 files import `db` outside `db/` and `service/` (. 1, admin 14, api 10, auth 2, cmd/gendocs 1, cmd/seed 2, internal/app 6, plugin 1, ws 22); 28 are type-only; 0 unlisted.
Dispositions: adapter 26, boundary 17, move 16. Move targets: auth 7, connection 2, upload 2, user 2, voice 3.
