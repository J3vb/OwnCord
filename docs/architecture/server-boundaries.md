...
| `adapter`   | a transport adapter that uses `db` types or pure helpers only — response shapes, status helpers — no persistence call |   37 |
...
no query of its own. The table is down from 59 rows to 57, and every row left
...
may read. 36 of the 57 rows are type-only, 28 of them `adapter`.
...
| `internal/app/maintenance.go`     | `DB×3`                                                                                                                                       | —                                                                            | `CleanupExpiredSecondFactorState` `DeleteExpiredSessions` `DeleteOrphanedAttachments`                                              | calls     | boundary    | —      | periodic worker: expired sessions, backups, orphan attachments                                                                                                                                |
...
57 files import `db` outside `db/` and `service/` (. 1, admin 12, api 11, auth 2, cmd/gendocs 1, cmd/seed 2, internal/app 7, plugin 1, ws 20); 36 are type-only; 0 unlisted.
Dispositions: adapter 37, boundary 20. Move targets: .

<!-- dbinventory:end -->

Reading the table:

- **Type-only rows (36, of which 28 are `adapter`)** mostly need no service:
