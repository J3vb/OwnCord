...
| Windows native (auth and UI)   | `npx playwright test --config playwright.config.native.ts --project native-no-auth --project native-authenticated` | Same executable: connect page and auth flows (unsaved), then a logged-in session covering layout, channel nav, chat, DMs, settings, appearance and overlays          |
