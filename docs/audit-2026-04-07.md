...
`HTTPDo` / `Storage*` / `EventSink.Dispatch` have no callers outside the
`plugin` package's own tests. The only guest-reachable entry points today are
`command_dispatch` (via the WS `chat_command` handler) and `list_commands` at
activation. Wiring those host imports is what makes #5 exploitable at all and
is the point at which #4's rate limit must exist.
