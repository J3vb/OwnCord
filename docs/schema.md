...
Generated from the migrated schema by `cd Server && go run -tags otel,wazero ./cmd/gendocs` — do not edit by hand; `make docs-verify` fails when it drifts. 48 tables: `sqlite_sequence` and the FTS5 shadow tables behind `messages_fts` are included; the `sqlite_stat*` tables `ANALYZE` writes are not, since they hold planner statistics and which of them exists depends on the SQLite build.
...
| `moderation_actions`       | `id INTEGER PK`, `kind TEXT NOT NULL`, `target_id INTEGER NOT NULL`, `actor_id INTEGER NOT NULL`, `actor_token TEXT`, `report_id INTEGER`, `reason TEXT NOT NULL`, `expires_at TEXT`, `acknowledged_at TEXT`, `lifted_at TEXT`, `lifted_by INTEGER NOT NULL`, `voice_muted INTEGER NOT NULL`, `created_at TEXT NOT NULL`                                                                                                                        | `idx_moderation_actions_report`, `idx_moderation_actions_target`, `idx_moderation_actions_timeouts`                                                 |
