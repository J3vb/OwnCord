...
| `044_user_storage.sql`                         | Adds `user_storage`, the durable per-user upload byte counter charged before every store write and recounted from the rows on the maintenance tick; adds `emoji.size` so emoji count beside attachments; seeds the counters from existing attachments (B5-2, SEC-04)                                                                                                                                                                   |
| `045_push_subscriptions.sql`                   | Adds `push_subscriptions` (one row per user/endpoint, `vapid_key_id` recording which VAPID key it was created under), storage only — nothing dispatches to it yet (B5-4, BG-05)                                                                                                                                                                                                                                                        |
