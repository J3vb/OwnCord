...
| Media frames are encrypted before they leave the machine; the SFU (LiveKit) relays ciphertext        | `Client/src/lib/livekitSession.ts`: the E2EE worker is created (`:424-426`) and `setE2EEEnabled(true)` is called on the room `createRoom` built (`:471`)                                                                                               | `livekit-session.test.ts` "enables E2EE on the room created by createRoom (OC-0095)"                                                                                              |
