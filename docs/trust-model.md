...
**Kind:** reference. **Verified against:** `dev` @ `012d5dbf`, 2026-09-17
(the voice table's citations were re-derived then and now name symbols first,
so a moved line does not silently make a row wrong).
...
| Rule                                                                                                 | Code                                                                                                                                                                                                                                                                    | Test                                                                                                                                                                              |
| ---------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| The room key is made on a participant's machine, never on the server                                 | `Client/src/lib/e2eeCrypto.ts` `generateRoomKey` (`:202`, WebCrypto random); called from `Client/src/lib/livekitE2EE.ts` in the key holder's `setupKeyExchange` path (`:293`) and in `features/voice/e2eeEpoch.ts` `rotateRoomKey`                                      | `livekit-e2ee.test.ts` "setupKeyExchange as key holder generates the room key and sends a signed announce"                                                                        |
...
| Media frames are encrypted before they leave the machine; the SFU (LiveKit) relays ciphertext        | `Client/src/features/voice/roomLifecycle.ts`: the E2EE worker is created (`:144`) and `setE2EEEnabled(true)` is called on the room `createRoom` built (`:197`)                                                                                                          | `livekit-session.test.ts` "enables E2EE on the room created by createRoom (OC-0095)"                                                                                              |
...
| The server holds no room key                                                                         | Absence: the only E2EE state on the server is each client's stored public key and `Hub.voiceKeyHolders` (`voice_e2ee.go` `updateKeyHolder` `:47`, `IsVoiceKeyHolder` `:77`, `getClientE2EEPubKey` `:278`); no decrypt path exists                                       | Pinned by the relay tests above; there is no positive "no key" assertion because there is nothing to assert on                                                                    |
| Each user has a long-lived identity key; peers pin it on first sight (TOFU) and block on a change    | `Client/src/lib/identity.ts` `storeIdentityPin` (`:116`), `getIdentityPin` (`:158`); decision in `features/voice/e2eePeerState.ts` `verifyPeerAnnounce` (a pinned peer whose published key differs is blocked, `:169`; first sight pins via `storeIdentityPin`, `:236`) | `identity.test.ts` "getIdentityPin reports a store error as 'unavailable', never 'unpinned' (DC-08)"; `livekit-e2ee.test.ts` "[finding 2] marks a first-sight peer 'unverified'…" |
| Accepting a new key re-pins the key the human saw, not a fresh read the server could have swapped    | `livekitE2EE.ts` `rePinPeerIdentity(userId, verifiedKey)` (`:549`)                                                                                                                                                                                                      | `livekit-session.test.ts` "re-pins the verified key, not a store re-read a malicious server mutated (TOCTOU)"                                                                     |
| When someone leaves, the key holder rotates the key, so the leaver cannot decrypt what comes next    | `livekitE2EE.ts` `handleParticipantLeft` (`:800`), `e2eeEpoch.ts` `rotateRoomKey` (`:116`)                                                                                                                                                                              | `livekit-session.test.ts` "rotates the room key when a keyed peer leaves while I stay key holder (forward secrecy)"                                                               |
| The key also rotates on a timer while the call runs                                                  | `features/voice/e2eeEpoch.ts` `rotateKeyPeriodically` (`:146`)                                                                                                                                                                                                          | `livekit-e2ee.test.ts` "[T-47] arms the periodic rotation timer for the key holder…"                                                                                              |
| A reconnect during a rotation gets the current key, not a stale one                                  | `Server/ws/hub_registry.go` `registerNow` (re-announce of the client's own key on resume, OC-0316; this lived in `hub.go` until `088c7eb7`)                                                                                                                             | `TestRegisterNow_ReannouncesOwnKeyOnResume`; client `"[OC-0007] confirms the room key after a reconnect re-announce…"`                                                            |
| The server still decides **who may join and publish** — E2EE hides content, not membership           | `Server/ws/livekit.go` `GenerateToken` (`:90`) with per-permission `CanPublishSources`; `Server/ws/voice_join.go` gates the join on `permissions.CanJoinVoice` (`:154`) and batches the per-source bits (`:382`)                                                        | `TestE2EE_VoiceToken_IncludesIsKeyHolder`; `Server/ws/voice_moderation_overrides_test.go`                                                                                         |
...
that key, and pins it (`Client/src/features/voice/e2eePeerState.ts` `verifyPeerAnnounce`,
the `storeIdentityPin` call). A malicious
...
(`Client/src/lib/identity.ts:11-12`; `features/voice/e2eePeerState.ts:143`, `:236`), while
...
A browser cannot inspect or pin a certificate. The browser adapter (post-beta)
...
| -------------------------- | ----------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Server database            | SQLite file at `database.path`                                                                  | **Not encrypted.** Plain `modernc.org/sqlite` driver, no cipher (`Server/db/db.go:17`, `:219`, DSN pragmas `:62-68`). Filesystem permissions and full-disk encryption are the operator's job                                                                            |
...
first-sight identity (`Client/src/features/voice/e2eePeerState.ts` `verifyPeerAnnounce`),
and the key holder wraps the room key to it (`handleAnnounceInner`, the
wrap-and-offer branch). Pinning every legitimate peer
...
| operator's OTLP collector (`telemetry.otlp_endpoint`)                                                                                                                                  | startup, only when `telemetry.exporter: otlp`                                               | traces and metrics to a collector the operator runs                                                                                                                                                                           | `telemetry.exporter: none` (default)                                                                                                                                                                                                                                                                               | `Server/config/config.go:148-166`; `Server/internal/app/telemetry.go:18`                                             |
| `127.0.0.1:<port>/health`                                                                                                                                                              | `healthcheck` subcommand (Docker `HEALTHCHECK`)                                             | liveness probe of itself                                                                                                                                                                                                      | n/a — loopback                                                                                                                                                                                                                                                                                                     | `Server/internal/app/healthcheck.go:24-94`                                                                           |
...
itself loopback `ws:`/`http:` (on Linux, where voice is native, any scheme
with a loopback host) (`Client/src/lib/livekitUrlResolver.ts` `resolve`, `direct_url`); LiveKit **media**
