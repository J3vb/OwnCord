...
| Each user has a long-lived identity key; peers pin it on first sight (TOFU) and block on a change    | `Client/src/lib/identity.ts` `storeIdentityPin` (`:116`), `getIdentityPin` (`:158`); decision in `livekitE2EE.ts` `verifyPeerAnnounce` (a pinned peer whose published key differs is blocked, `:594`; first sight pins via `storeIdentityPin`, `:661`) | `identity.test.ts` "getIdentityPin reports a store error as 'unavailable', never 'unpinned' (DC-08)"; `livekit-e2ee.test.ts` "[finding 2] marks a first-sight peer 'unverified'…" |
| Accepting a new key re-pins the key the human saw, not a fresh read the server could have swapped    | `livekitE2EE.ts` `rePinPeerIdentity(userId, verifiedKey)` (`:716`)                                                                                                                                                                                     | `livekit-session.test.ts` "re-pins the verified key, not a store re-read a malicious server mutated (TOCTOU)"                                                                     |
| When someone leaves, the key holder rotates the key, so the leaver cannot decrypt what comes next    | `livekitE2EE.ts` `handleParticipantLeft` (`:1307`), `rotateRoomKey` (`:1289`)                                                                                                                                                                          | `livekit-session.test.ts` "rotates the room key when a keyed peer leaves while I stay key holder (forward secrecy)"                                                               |
| The key also rotates on a timer while the call runs                                                  | `livekitE2EE.ts` `rotateKeyPeriodically` (`:1525`)                                                                                                                                                                                                     | `livekit-e2ee.test.ts` "[T-47] arms the periodic rotation timer for the key holder…"                                                                                              |
...
that key, and pins it (`Client/src/lib/livekitE2EE.ts` `verifyPeerAnnounce`,
...
first-sight identity (`Client/src/lib/livekitE2EE.ts` `verifyPeerAnnounce`),
